...
export default function AdminDashboardPage() {
  ...
  const t = {
    fr: {
      ...
      effectivenessTitle: "Content Effectiveness Scoring",
      effectivenessBtn: "Analyser l'efficacité",
      effectivenessAnalyzing: "Analyse...",
      effectivenessScore: "Score",
      effectivenessKpiTitle: "KPI Analysis",
      effectivenessTimeTitle: "Time to Mastery",
      effectivenessDropTitle: "Drop-off Analysis",
      effectivenessTaskCompletion: "Complétion tâche",
      effectivenessKpiMeasurement: "Mesure KPI",
      effectivenessAvgTime: "Temps moyen",
      effectivenessAssessment: "Assessment",
      effectivenessCriticalSection: "Section critique",
      effectivenessDropRate: "Taux abandon",
      effectivenessRecommendations: "Recommandations d'amélioration",
      effectivenessFlags: "Alertes",
      effectivenessModuleId: "Module ID",
    },
  };
  const tx = t[lang];
  ...
  const exportCSV = () => {
    if (!overviewData && !roleData && !effectivenessData) {
      alert("Chargez au moins une section avant d'exporter.");
      return;
    }
    const rows = [];
      ...
      <div className="mx-auto max-w-page">
        ...
        <section className="rounded-3xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-6 shadow-sm">
          ...
          <div className="mt-4 flex gap-3">
            <button onClick={exportCSV} className="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
              ⬇ Export CSV
            </button>
            <button onClick={exportPDF} className="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
              🖨 Export PDF
            </button>
          </div>
        </section>
        ...
        <section className="mt-6">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-bold text-euk-dark">Alertes automatiques</h2>
            <button
              onClick={loadAlerts}
          ...
          {alertsData && !alertsLoading && (
            <div className="space-y-4">
              <div className="grid gap-4 md:grid-cols-3">
                <KpiCard label="Total alertes" value={alertsData.total_alerts} color="text-slate-700" />
                <KpiCard label="🔴 Critiques" value={alertsData.critical_count} color="text-red-600" />
                <KpiCard label="🟡 À surveiller" value={alertsData.medium_count} color="text-amber-600" />
              </div>

              {alertsData.critical?.length > 0 && (
                <div className="rounded-3xl border border-red-200 bg-white p-6 shadow-sm">
                  <h3 className="text-base font-bold text-red-700 mb-4">🔴 Modules critiques</h3>
                  <div className="space-y-3">
                    {alertsData.critical.map((a, i) => (
                      <div key={i} className="rounded-2xl border border-red-100 bg-red-50 p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1">
                            <div className="text-sm font-bold text-euk-dark">{a.module_title}</div>
                            <div className="text-xs text-slate-500 mt-0.5">{a.role} • {a.level} • {a.learners_count} apprenant(s)</div>
                            <div className="mt-2 text-xs font-semibold text-red-700">⚠️ {a.message}</div>
                          </div>
                          <div className="shrink-0 text-right space-y-1">
                            <div className="text-xs text-slate-500">Progression : <span className="font-bold text-euk-dark">{a.avg_progress}%</span></div>
                            <div className="text-xs text-slate-500">Mastery : <span className="font-bold text-euk-dark">{a.avg_mastery}%</span></div>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              {alertsData.medium?.length > 0 && (
                <div className="rounded-3xl border border-amber-200 bg-white p-6 shadow-sm">
                  <h3 className="text-base font-bold text-amber-700 mb-4">🟡 Modules à surveiller</h3>
                  <div className="space-y-3">
                      ...
                      <div key={i} className="rounded-2xl border border-amber-100 bg-amber-50 p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1">
                            <div className="text-sm font-bold text-euk-dark">{a.module_title}</div>
                            <div className="text-xs text-slate-500 mt-0.5">{a.role} • {a.level} • {a.learners_count} apprenant(s)</div>
                            <div className="mt-2 text-xs font-semibold text-amber-700">⚠️ {a.message}</div>
                          </div>
                          <div className="shrink-0 text-right space-y-1">
                            <div className="text-xs text-slate-500">Complétion : <span className="font-bold text-emerald-600">{a.completion_rate}%</span></div>
                            <div className="text-xs text-slate-500">Mastery : <span className="font-bold text-euk-dark">{a.avg_mastery}%</span></div>
                          </div>
                        </div>
                      </div>
              ...
              {alertsData.total_alerts === 0 && (
                <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6 text-center">
                  <div className="text-emerald-700 font-semibold">✅ Aucune alerte détectée — tous les modules sont dans les seuils normaux.</div>
                </div>
              )}
            </div>
          )}
                  ...
                  <div className="space-y-2">
                    {overviewData.top_modules.map((m, i) => (
                      <div key={i} className="flex items-center gap-4 rounded-2xl border border-slate-100 bg-slate-50 p-3">
                        <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-euk-primary/10 text-xs font-bold text-euk-primary">{i + 1}</div>
            ...
            <div className="space-y-4 mt-4">
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <p className="flex-1 text-sm leading-6 text-slate-700">{roleData.insights.summary}</p>
                    ...
                    {roleData.insights.top_blockers.map((b, i) => (
                      <div key={i} className="flex items-start gap-4 rounded-2xl border border-slate-100 bg-slate-50 p-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="mt-1 text-xs text-slate-500"><span className="font-medium">Evidence : </span>{b.evidence}</div>
                          <div className="mt-1 text-xs text-slate-500"><span className="font-medium">Impact : </span>{b.impact}</div>
                        </div>
                      </div>
                    ))}
                  </div>
...
onClick={async () => {
    ...
    try {
        const res = await apiFetch(`/api/v1/analytics/module/${moduleId}`);
        if (res?.ok) setEffectivenessData(await res.json());
    } catch { /* non-blocking */ }
    finally { setEffectivenessLoading(false); }
}}
  disabled={moduleLoading || effectivenessLoading}
  ...
  {(moduleLoading || effectivenessLoading) ? tx.moduleLoading : tx.moduleBtn}
</button>
            </div>
          </div>
          ...
          {moduleData && !moduleLoading && (
            <div className="space-y-4">
              ...
              {moduleData.kpi_summary?.length > 0 && (
                <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                  <h3 className="text-base font-bold text-euk-dark mb-4">KPI Cohorte — Impact business</h3>
                  <div className="space-y-3">
                    {moduleData.kpi_summary.map((kpi, i) => (
                      <div key={i} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-sm font-semibold text-euk-dark">{kpi.indicator}</span>
                          <span className="text-xs text-slate-500">{kpi.learners_measured}/{kpi.learners_total} apprenants mesurés</span>
                        </div>
                        <div className="grid grid-cols-3 gap-3 text-xs">
                          <div className="rounded-xl bg-white border border-slate-200 p-2 text-center">
                            ...
                            <div className="font-bold text-euk-dark">{kpi.baseline_avg ?? "—"} {kpi.unit}</div>
                          </div>
                          <div className="rounded-xl bg-white border border-slate-200 p-2 text-center">
                            <div className="text-slate-500 mb-1">Actuel moy.</div>
                            <div className="font-bold text-euk-dark">{kpi.current_avg != null ? `${kpi.current_avg} ${kpi.unit}` : "—"}</div>
                          </div>
                          <div className="rounded-xl bg-white border border-slate-200 p-2 text-center">
                            <div className="text-slate-500 mb-1">Delta moy.</div>
                            <div className={`font-bold ${kpi.delta_avg_pct == null ? "text-slate-400" : kpi.delta_avg_pct < 0 ? "text-emerald-600" : "text-blue-600"}`}>
                              {kpi.delta_avg_pct != null ? `${kpi.delta_avg_pct}%` : "—"}
                            </div>
                          </div>
                        </div>
                        <div className="mt-2 text-xs text-slate-500">🎯 Cible : {kpi.target_label}</div>
                        <div className="mt-2 flex items-center gap-2">
                          <div className="h-1.5 flex-1 rounded-full bg-slate-200">
                            <div className="h-1.5 rounded-full bg-euk-primary" style={{ width: `${Math.round(kpi.measurement_rate * 100)}%` }} />
                          </div>
                          <span className="text-xs text-slate-500 shrink-0">{Math.round(kpi.measurement_rate * 100)}% saisi</span>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  {moduleData.insights?.kpi_insights && (
                    <div className="mt-4 rounded-2xl border border-violet-100 bg-violet-50 p-4">
                      <div className="text-xs font-bold uppercase tracking-wide text-violet-700 mb-2">Analyse IA</div>
                      <p className="text-xs text-slate-700 leading-5">{moduleData.insights.kpi_insights.comment}</p>
...
<section className="mt-8 mb-8">
  <div className="mb-4">
    <h2 className="text-lg font-bold text-euk-dark">{tx.effectivenessTitle}</h2>
    <p className="mt-1 text-sm text-slate-500">
      Lancé automatiquement avec l'analyse du module ci-dessus.
    </p>
  </div>
  ...
  {effectivenessLoading && <Spinner color="border-emerald-600" />}
          ...
          {effectivenessData && !effectivenessLoading && (
            <div className="space-y-4">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-xs text-slate-500">{tx.effectivenessScore}</span>
                    </div>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="text-xs font-bold uppercase tracking-wide text-slate-400 mb-3">{tx.effectivenessKpiTitle}</div>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-slate-500">{tx.effectivenessTaskCompletion}</span>
                      <span className="font-bold text-euk-primary">
                        {Math.round((effectivenessData.analysis?.kpi_analysis?.completion_rate || 0) * 100)}%
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">{tx.effectivenessKpiMeasurement}</span>
                      <span className="font-bold text-euk-primary">
                ...
                <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="text-xs font-bold uppercase tracking-wide text-slate-400 mb-3">{tx.effectivenessTimeTitle}</div>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-slate-500">{tx.effectivenessAvgTime}</span>
                      <span className="font-bold text-euk-primary">
                    ...
                    <div className="flex justify-between">
                      <span className="text-slate-500">{tx.effectivenessAssessment}</span>
                ...
                <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="text-xs font-bold uppercase tracking-wide text-slate-400 mb-3">{tx.effectivenessDropTitle}</div>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-slate-500">{tx.effectivenessCriticalSection}</span>
                      <span className="font-bold text-red-600">
                        {effectivenessData.analysis?.drop_off_analysis?.main_drop_off_section ?? "—"}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">{tx.effectivenessDropRate}</span>
                      <span className="font-bold text-red-600">
              ...
              {effectivenessData.analysis?.recommendations?.length > 0 && (
                <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                  <h3 className="text-base font-bold text-euk-dark mb-4">{tx.effectivenessRecommendations}</h3>
                  <div className="grid gap-3 md:grid-cols-3">
