...
from app.schemas.diagnostic_submit import SubmitAnswersIn
from app.schemas.diagnostic_results import SkillResultOut, DiagnosticResultsOut
from app.schemas.diagnostic_questionnaire import QuestionnaireOut
...
def _cooldown_check(db: Session, user_id: int, career_path_id: int):
    ...
    next_allowed_at = last_session.created_at + timedelta(days=settings.diagnostic_COOLDOWN_DAYS)
...
def get_questionnaire(
    ...
    if not skills:
        raise HTTPException(
            status_code=404,
            detail=f"No diagnostic found for career_path_id={current_user.career_path_id}"
        )
...
def submit_answers(
    ...
    if cooldown["has_scores"] and not cooldown["eligible"]:
        raise HTTPException(
            status_code=403,
            detail={
                "message": f"You can retake the diagnostic every {settings.diagnostic_COOLDOWN_DAYS} days.",
                "last_diagnostic_at": _dt_iso(cooldown["last"]),
    ...
    try:
        ...
        for sid, correct in correct_per_skill.items():
            ...
            score = round(correct * 100 / total_q) if total_q > 0 else 0
        ...
        if not results:
            raise RuntimeError("No skill scores computed — refusing to commit orphan session")
        ...
        logger.info(
            f"Diagnostic submitted: user_id={current_user.id}, "
        ...
        db.rollback()
        logger.error(
            f"Diagnostic submit failed for user_id={current_user.id}: {e}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
            detail="Failed to save diagnostic results. Please try again."
        )
...
def get_results(
    ...
    last_session = db.execute(
        select(DiagnosticSession)
        .where(DiagnosticSession.user_id == current_user.id)
        .where(DiagnosticSession.career_path_id == current_user.career_path_id)
        .order_by(desc(DiagnosticSession.created_at))
        .limit(1)
    ).scalar_one_or_none()
...
def diagnostic_status(
    ...
    global_score = None
    if last_session:
        expires_at = last_session.created_at + timedelta(days=settings.diagnostic_COOLDOWN_DAYS)
